	...
	C::Api: AuthorFilterAPI<B, NimbusId>,
{
	...
	let prediction_helper = |at, nimbus_id: NimbusId, slot: u32, parent| -> bool {
		let _has_nimbus_api = client
			.runtime_api()
			.has_api::<dyn NimbusApi<B>>(at)
			.expect("should be able to dynamically detect the api");

        // Our current runtime supports the old NimbusApi, therefore this will evaluate to false.
        // Unfortunately, we deactivated the AuthorFilterAPI. Consequently the client will
        // panic because it can't find an appropriate API to author the block. For now,
        // we'll evaluate the expression to true because we know that the API is present.
        // This is a hotfix and MUST be undone once the next runtime is deployed.
		if /*has_nimbus_api*/ true {
