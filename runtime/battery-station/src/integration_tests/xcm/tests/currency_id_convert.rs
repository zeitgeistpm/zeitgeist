...
use super::{reset_test_net, with_battery_station};
use crate::{
...
{
    let _test_net = reset_test_net();
    with_battery_station(|| assert_eq!(<AssetConvert as Convert<_, _>>::convert(asset), None));
}
