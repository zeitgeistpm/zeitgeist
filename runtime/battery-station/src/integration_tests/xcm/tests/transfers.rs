...
fn transfer_roc_from_relay_chain() {
    ...
    RococoNet::execute_with(|| {
        let initial_balance = rococo_runtime::Balances::free_balance(<[u8; 32] as std::convert::Into<RococoNet::AccountId>>::into(ALICE));
...
fn transfer_roc_to_relay_chain() {
    ...
    RococoNet::execute_with(|| {
        assert_eq!(rococo_runtime::Balances::free_balance(<[u8; 32] as std::convert::Into<RococoNet::AccountId>>::into(ALICE)), 999_988_806_429);
    });
}
...
fn transfer_ztg_to_sibling_with_custom_fee() {
    ...
    Zeitgeist::execute_with(|| {
        assert_eq!(Balances::free_balance(<[u8; 32] as std::convert::Into<Balances::AccountId>>::into(ALICE)), alice_initial_balance);
        assert_eq!(Balances::free_balance(sibling_parachain_account()), 0);
        assert_ok!(XTokens::transfer(
            RuntimeOrigin::signed(ALICE.into()),
            CurrencyId::Ztg,
