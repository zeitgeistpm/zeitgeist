...
use crate::{
    ...
    AssetRegistry, Balance, Balances, CurrencyId, Origin, Tokens, XTokens, ExistentialDeposit
};
...
fn transfer_ztg_to_sibling() {
	...
	let ztg_in_sibling = CurrencyId::ForeignAsset(12);
	...
	Sibling::execute_with(|| {
		assert_eq!(
			OrmlTokens::free_balance(ztg_in_sibling.clone(), &BOB.into()),
			0
		);
