...
impl SubstrateCli for Cli {
    ...
    fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
        load_spec(id, self.run.parachain_id.unwrap_or(crate::DEFAULT_PARACHAIN_ID).into())
    }
...
pub fn load_spec(
    ...
    Ok(match id {
        "" | "dev" => Box::new(crate::chain_spec::dev_config(parachain_id)?),
        "battery_station" => Box::new(crate::chain_spec::ChainSpec::from_json_bytes(
            &include_bytes!("../res/battery-station.json")[..],
        )?),
        "battery_station_staging" => {
            Box::new(crate::chain_spec::battery_station_staging_config(parachain_id)?)
        }
        path => {
