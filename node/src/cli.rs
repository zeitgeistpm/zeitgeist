...
impl ClientHandle for Client {
	fn execute_with<T: ExecuteWithClient>(&self, t: T) -> T::Output {
		match self {
            #[cfg(feature = "with-battery-station-runtime")]
			Self::BatteryStation(client) => T::execute_with_client::<_, _, FullBackend>(t, client.clone()),
			#[cfg(feature = "with-zeitgeist-runtime")]
			Self::Zeitgeist(client) => T::execute_with_client::<_, _, FullBackend>(t, client.clone()),
		}
	}
}
