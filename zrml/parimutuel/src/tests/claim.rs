...
fn claim_rewards_emits_event() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let loser_asset = ParimutuelAsset::Share(market_id, 1u16);
...
fn claim_rewards_categorical_changes_balances_correctly() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let free_winner_asset_alice_before =
            AssetManager::free_balance(winner_asset.into(), &ALICE);
        ...
        let free_base_asset_alice_before =
            AssetManager::free_balance(market.base_asset.into(), &ALICE);
        let free_base_asset_pot_before = AssetManager::free_balance(
            market.base_asset.into(),
            &Parimutuel::pot_account(market_id),
        );
        ...
        assert_eq!(
            free_winner_asset_alice_before
                - AssetManager::free_balance(winner_asset.into(), &ALICE),
        ...
        let free_winner_asset_charlie_before =
            AssetManager::free_balance(winner_asset.into(), &CHARLIE);
        ...
        let free_base_asset_charlie_before =
            AssetManager::free_balance(market.base_asset.into(), &CHARLIE);

        assert_ok!(Parimutuel::claim_rewards(RuntimeOrigin::signed(CHARLIE), market_id));

        assert_eq!(
            free_winner_asset_charlie_before
                - AssetManager::free_balance(winner_asset.into(), &CHARLIE),
...
fn claim_refunds_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let bob_asset = ParimutuelAsset::Share(market_id, 1u16);
        ...
        let free_base_asset_alice_before =
            AssetManager::free_balance(market.base_asset.into(), &ALICE);
        let free_base_asset_bob_before = AssetManager::free_balance(market.base_asset.into(), &BOB);
        let free_base_asset_pot_before = AssetManager::free_balance(
            market.base_asset.into(),
            &Parimutuel::pot_account(market_id),
        );
        ...
        assert_eq!(
            AssetManager::free_balance(market.base_asset.into(), &BOB) - free_base_asset_bob_before,
            bob_amount_minus_fees
        );
        assert_eq!(
            AssetManager::free_balance(
