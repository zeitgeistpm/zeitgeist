...
// TODO Rename to `matching_y_coordinate`.
...
mod tests {
    ...
    #[test_case(
        "0xede9f77d62bb62fbe81b267435fcf72fd0251b2814d10df320f6ea215eed56",
        ""
    )]
    #[test_case(
    ...
    fn quadratic_residue_works(x: &str, expected: &str) {
        let x = Fq::from_str_prefixed(x).unwrap();
        let expected =
            if expected.is_empty() { None } else { Some(Fq::from_str_prefixed(expected).unwrap()) };

        let result = quadratic_residue(x);
