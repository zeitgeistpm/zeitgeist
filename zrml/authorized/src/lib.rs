...
mod pallet {
    use crate::{
        types::SetPalletConstantsParams, weights::WeightInfoZeitgeist, AuthorizedPalletApi,
    };
    use alloc::vec::Vec;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn authorize_market_outcome(
            ...
            let (report, ids_len) = match &report_opt {
                ...
                None => {
                    let resolve_at = now.saturating_add(T::PalletConstants::get().correction_period);
        ...
        #[pallet::call_index(1)]
        #[pallet::weight(T::WeightInfo::set_pallet_constants())]
        #[frame_support::transactional]
        pub fn set_pallet_constants(
            origin: OriginFor<T>,
            params: SetPalletConstantsParams<T>,
        ) -> DispatchResult {
            let _ = T::SetPalletConstantsOrigin(origin)?;
            T::PalletConstants::try_mutate(|pallet_constants| {
                pallet_constants.set(params).map_err(|e| Error::<T>::IllegalPalletConstants(e))
            })
        }
    }
    ...
    pub trait Config: frame_system::Config {
        ...
        type SetPalletConstantsOrigin: EnsureOrigin<Self::RuntimeOrigin>;
    ...
    pub enum Error<T> {
        ...
        OutcomeMismatch,
        /// The specified pallet constants are not allowed.
        IllegalPalletConstants,
    }
    ...
    #[pallet::storage]
    pub type PalletConstants<T: Config> =
        StorageValue<_, PalletConstants, ValueQuery, DefaultSettableConstants<T>>;

    #[pallet::type_value]
    pub fn DefaultPalletConstants<T: Config>() -> PalletConstants<T> {
        PalletConstants { correction_period: T::CorrectionPeriod::get() }
    }
