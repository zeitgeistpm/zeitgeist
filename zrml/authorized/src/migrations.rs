...
use zeitgeist_primitives::types::{AuthorityReport, OutcomeReport};
...
impl<T: Config + zrml_market_commons::Config> OnRuntimeUpgrade for AddFieldToAuthorityReport<T> {
    fn on_runtime_upgrade() -> Weight
    where
        T: Config,
    {
        let mut total_weight = T::DbWeight::get().reads(1);
        let authorized_version = StorageVersion::get::<Pallet<T>>();
            ...
            storage_iter::<Option<OutcomeReport>>(AUTHORIZED, AUTHORIZED_OUTCOME_REPORTS)
        {
            ...
            if let Some(outcome) = old_value {
                let resolve_at: T::BlockNumber =
                    now.saturating_add(<T as Config>::ReportPeriod::get());
        ...
        StorageVersion::new(AUTHORIZED_NEXT_STORAGE_VERSION).put::<Pallet<T>>();
        total_weight = total_weight.saturating_add(T::DbWeight::get().writes(1));
...
mod tests_authorized {
    ...
    fn on_runtime_upgrade_increments_the_storage_versions() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let authorized_version = StorageVersion::get::<Pallet<Runtime>>();
    ...
    fn on_runtime_sets_new_struct_with_resolve_at() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let resolve_at: <Runtime as frame_system::Config>::BlockNumber =
                now.saturating_add(<Runtime as Config>::ReportPeriod::get());
    ...
    fn on_runtime_is_noop_if_versions_are_not_correct() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            StorageVersion::new(AUTHORIZED_NEXT_STORAGE_VERSION).put::<Pallet<Runtime>>();
    ...
    fn set_up_chain() {
        StorageVersion::new(AUTHORIZED_REQUIRED_STORAGE_VERSION).put::<Pallet<Runtime>>();
    }
}
