...
fn vote_fails_if_insufficient_amount() {
    ExtBuilder::default().build().execute_with(|| {
        let market_id = 0u128;
        let vote_id = 0u8;
        let id = (&market_id, &vote_id);
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(0), 10 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(20), 20 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(40), 30 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(60), 40 * BASE));
...
fn get_voting_winner_sets_the_last_outcome_for_same_vote_balances_as_the_canonical_outcome() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let id = (&market_id, &vote_id);
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(0), 10 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(20), 10 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(40), 10 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(60), 10 * BASE));
        ...
        assert_eq!(&GlobalDisputes::get_voting_winner(id).unwrap(), &OutcomeReport::Scalar(60));
    });
}
...
fn get_voting_winner_sets_the_highest_vote_of_outcome_markets_as_the_canonical_outcome() {
    ExtBuilder::default().build().execute_with(|| {
        let market_id = 0u128;
        let mut latest_vote_id = 0u8;
        let reinitialize_outcomes = |id| {
            assert_ok!(GlobalDisputes::push_voting_outcome(
                id,
                OutcomeReport::Scalar(0),
                100 * BASE
        ...
        latest_vote_id += 1;
        let id = (&market_id, &latest_vote_id.clone());
        reinitialize_outcomes(id);
        assert_ok!(GlobalDisputes::vote_on_outcome(
            Origin::signed(ALICE),
            market_id,
            latest_vote_id,
        ...
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 0u32).unwrap(), 110 * BASE);
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 1u32).unwrap(), 110 * BASE);
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 2u32).unwrap(), 111 * BASE);
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 3u32).unwrap(), 110 * BASE);

        assert_eq!(GlobalDisputes::get_voting_winner(id).unwrap(), OutcomeReport::Scalar(40));

        assert_eq!(<Outcomes<Runtime>>::get(id), vec![]);
        ...
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 0u32).unwrap(), 100 * BASE);
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 1u32).unwrap(), 150 * BASE);
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 2u32).unwrap(), 100 * BASE);
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 3u32).unwrap(), 151 * BASE);
        ...
        assert_eq!(GlobalDisputes::get_voting_winner(id).unwrap(), OutcomeReport::Scalar(0));
        ...
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 1u32).unwrap(), 101 * BASE);
        ...
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 0u32).unwrap(), 150 * BASE);
        ...
        assert_eq!(<OutcomeVotes<Runtime>>::get(id, 1u32).unwrap(), 140 * BASE);
...
fn get_voting_winner_clears_outcome_votes() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let vote_id = 0u8;
        let id = (&market_id, &vote_id);
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(0), 0 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(20), 0 * BASE));
...
fn unlock_clears_lock_info() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert!(<LockInfoOf<Runtime>>::get(ALICE, id).is_none());
    });
}
...
fn vote_fails_if_outcome_len_below_min_outcomes() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(GlobalDisputes::push_voting_outcome(id, OutcomeReport::Scalar(0), 42 * BASE));
...
fn locking_works_for_one_market() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(LockInfoOf::<Runtime>::get(ALICE, id), None);
        assert!(Balances::locks(ALICE).is_empty());
        assert_eq!(LockInfoOf::<Runtime>::get(BOB, id), None);
        assert!(Balances::locks(BOB).is_empty());
        assert_eq!(LockInfoOf::<Runtime>::get(CHARLIE, id), None);
        assert!(Balances::locks(CHARLIE).is_empty());
        assert_eq!(LockInfoOf::<Runtime>::get(EVE, id), None);
        ...
        assert_eq!(LockInfoOf::<Runtime>::get(BOB, id), Some((1u32, 40 * BASE)));
        assert_eq!(Balances::locks(BOB), vec![the_lock(40 * BASE)]);
        assert_eq!(LockInfoOf::<Runtime>::get(CHARLIE, id), Some((2u32, 30 * BASE)));
...
fn locking_works_for_two_markets_with_stronger_first_unlock() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let vote_id = 0u8;
        let id_1 = (&market_id_1, &vote_id);
        let market_id_2 = 1u128;
        let id_2 = (&market_id_2, &vote_id);
        assert_ok!(GlobalDisputes::push_voting_outcome(id_1, OutcomeReport::Scalar(0), 10 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id_1, OutcomeReport::Scalar(20), 20 * BASE));

        assert_ok!(GlobalDisputes::push_voting_outcome(id_2, OutcomeReport::Scalar(0), 10 * BASE));
        assert_ok!(GlobalDisputes::push_voting_outcome(id_2, OutcomeReport::Scalar(20), 20 * BASE));

        assert_eq!(LockInfoOf::<Runtime>::get(ALICE, id_1), None);
        assert_eq!(LockInfoOf::<Runtime>::get(ALICE, id_2), None);
        assert!(Balances::locks(ALICE).is_empty());
        assert_eq!(LockInfoOf::<Runtime>::get(BOB, id_1), None);
        assert_eq!(LockInfoOf::<Runtime>::get(BOB, id_2), None);
        assert!(Balances::locks(BOB).is_empty());
        ...
        assert_eq!(Balances::locks(ALICE), vec![the_lock(50 * BASE)]);
        assert_eq!(LockInfoOf::<Runtime>::get(BOB, id_1), Some((1u32, 40 * BASE)));
        assert_eq!(LockInfoOf::<Runtime>::get(BOB, id_2), Some((1u32, 20 * BASE)));
        assert_eq!(Balances::locks(BOB), vec![the_lock(40 * BASE)]);
