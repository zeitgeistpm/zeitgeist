...
fn allows_the_full_user_lifecycle() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(Swaps::pool_join(
            ...
            0,
            FIXEDU128_5,
            vec!(FIXEDU128_25, FIXEDU128_25, FIXEDU128_25, FIXEDU128_25),
        ));
        ...
        assert_eq!(expected_in, from_integral(10_290_319_622));
        ...
        assert_eq!(asset_b_bal_after_2 - asset_b_bal_after, from_parts(1, 0));
    });
}
...
fn create_initial_pool() {
    ASSETS.iter().cloned().for_each(|asset| {
        let _ = Shares::generate(asset, &BOB, FIXEDU128_100);
    });
    assert_ok!(Swaps::create_pool(
        Origin::signed(BOB),
        ASSETS.iter().cloned().collect(),
        vec![FIXEDU128_2, FIXEDU128_2, FIXEDU128_2, FIXEDU128_2],
    ));
}

fn create_initial_pool_with_funds_for_alice() {
    create_initial_pool();
    let _ = Shares::generate(ASSET_A, &ALICE, FIXEDU128_25);
    let _ = Shares::generate(ASSET_B, &ALICE, FIXEDU128_25);
    let _ = Shares::generate(ASSET_C, &ALICE, FIXEDU128_25);
    let _ = Shares::generate(ASSET_D, &ALICE, FIXEDU128_25);
}

fn event_exists(raw_evt: crate::RawEvent<AccountId>) -> bool {
