...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        #[weight = 0]
        #[frame_support::transactional]
        fn pool_exit(origin, pool_id: u128, pool_amount: BalanceOf<T>, min_assets_out: Vec<BalanceOf<T>>) -> DispatchResult {
            (|| {
                pool!(
        ...
        #[weight = 0]
        #[frame_support::transactional]
        fn pool_join(origin, pool_id: u128, pool_amount: BalanceOf<T>, max_assets_in: Vec<BalanceOf<T>>) -> DispatchResult {
            (|| {
                pool!(
