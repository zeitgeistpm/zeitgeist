...
use frame_support::{
    ...
    pallet_prelude::{
        ConstU32, DispatchResultWithPostInfo, EnsureOrigin, Hooks, MaxEncodedLen, Member,
        OptionQuery, StorageMap, StorageValue, ValueQuery, Weight,
    },
    traits::{
    ...
    transactional, Blake2_128Concat, BoundedVec, PalletId, Parameter,
};
use frame_system::{
    ensure_signed,
...
mod pallet {
    ...
    pub trait Config: frame_system::Config {
        ...
        /// The vote item to be used for the court.
        type VoteItem: Parameter
            + Member
            + Clone
            + Ord
            + PartialEq
    ...
    pub(crate) type CourtOf<T> =
        CourtInfo<<T as frame_system::Config>::BlockNumber, AppealsOf<T>, <T as Config>::VoteItem>;
    ...
    pub(crate) type VoteOf<T> = Vote<HashOf<T>, DelegatedStakesOf<T>, <T as Config>::VoteItem>;
    pub(crate) type JurorVoteWithStakesOf<T> =
        JurorVoteWithStakes<AccountIdOf<T>, BalanceOf<T>, <T as Config>::VoteItem>;
    ...
    pub(crate) type DrawOf<T> = Draw<
        ...
        HashOf<T>,
        DelegatedStakesOf<T>,
        <T as Config>::VoteItem,
    >;
    pub(crate) type SelectedDrawsOf<T> = BoundedVec<DrawOf<T>, <T as Config>::MaxSelectedDraws>;
    pub(crate) type AppealOf<T> = AppealInfo<AccountIdOf<T>, BalanceOf<T>, <T as Config>::VoteItem>;
    pub(crate) type AppealsOf<T> = BoundedVec<AppealOf<T>, <T as Config>::MaxAppeals>;
    pub(crate) type CommitmentMatcherOf<T> =
        CommitmentMatcher<AccountIdOf<T>, HashOf<T>, <T as Config>::VoteItem>;
    pub(crate) type RawCommitmentOf<T> =
        RawCommitment<AccountIdOf<T>, HashOf<T>, <T as Config>::VoteItem>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn denounce_vote(
            ...
            let market = T::MarketCommons::market(&market_id)?;
                ...
                market.matches_outcome_report(&vote_item.clone().into()),
        ...
        pub fn appeal(
            ...
            let appealed_vote_item =
                Self::get_latest_winner_vote_item(&market_id, old_draws.as_slice())?;
        ...
        T: Config,
    {
        ...
        pub(crate) fn get_winner(
            draws: &[DrawOf<T>],
            last_winner: Option<T::VoteItem>,
        ) -> Option<T::VoteItem> {
            let mut scores = BTreeMap::<T::VoteItem, u32>::new();
        ...
        // TODO make this independent / generic from the market and oracle outcome.
        ...
        pub(crate) fn get_latest_winner_vote_item(
            market_id: &MarketIdOf<T>,
            last_draws: &[DrawOf<T>],
        ) -> Result<T::VoteItem, DispatchError> {
            let market = T::MarketCommons::market(market_id)?;
            let court = <Courts<T>>::get(market_id).ok_or(Error::<T>::CourtNotFound)?;
            let last_winner: Option<T::VoteItem> = court
                .appeals
                .last()
                .map(|appeal_info| Some(appeal_info.appealed_vote_item.clone()))
                .unwrap_or(None);
            // TODO use some generic form to input the default vote item (Query with a trait)
            ...
            let winner_vote_item =
                Self::get_winner(last_draws, last_winner).unwrap_or_else(|| oracle_outcome.into());
            Ok(winner_vote_item)
        }
        ...
        T: Config,
    {
        ...
        fn on_resolution(
            ...
            let draws = SelectedDraws::<T>::get(market_id);
            let draws_len = draws.len() as u32;
            let winner_vote_item = Self::get_latest_winner_vote_item(market_id, draws.as_slice())?;
            Self::unlock_jurors_from_last_draw(market_id, draws);
            court.status = CourtStatus::Closed { winner: winner_vote_item.clone() };
            <Courts<T>>::insert(market_id, court);

            let res = ResultWithWeightInfo {
                result: Some(winner_vote_item.into()),
        ...
        fn exchange(
            ...
            for AppealInfo { backer, bond, appealed_vote_item } in &court.appeals {
                let appealed_vote_item_as_outcome = appealed_vote_item.clone().into();
        ...
        > {
            ...
            let gd_outcomes = court
                ...
                .filter(|a| &a.appealed_vote_item.clone().into() != oracle_outcome)
                .map(|a| {
                    GlobalDisputeItem {
                        outcome: a.appealed_vote_item.clone().into(),
