use crate::{Error, mock::*, OrderSide};
use frame_support::{
...
fn it_makes_orders() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let shares_reserved = <Shares as ReservableShares<AccountId, Balance, H256>>::reserved_balance(H256::repeat_byte(1), &BOB);
...
fn it_takes_orders() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let alice_shares = <Shares as SharesTrait<AccountId, Balance, H256>>::free_balance(shares_id, &ALICE);
        assert_eq!(alice_bal, 950);
        assert_eq!(alice_shares, 10);

        let bob_bal = <Balances as Currency<AccountId>>::free_balance(&BOB);
        let bob_shares = <Shares as SharesTrait<AccountId, Balance, H256>>::free_balance(shares_id, &BOB);
